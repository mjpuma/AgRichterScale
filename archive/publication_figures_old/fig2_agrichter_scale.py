...
from lib.agririchter_viz import AgriRichterScaleVisualizer
import pandas as pd
...
def load_real_events(crop: str, config: Config, grid_manager: GridDataManager):
    ...
    try:
        ...
        if not country_file.exists() or not state_file.exists():
            logger.warning("  Event Excel files not found, using sample data")
            from lib.agririchter_viz import create_sample_events_data
            return create_sample_events_data(crop)
        ...
        logger.warning(f"  Failed to load real events: {e}, using sample data")
        from lib.agririchter_viz import create_sample_events_data
        return create_sample_events_data(crop)
...
def main():
    ...
    logger.info("=" * 60)
    logger.info("FIGURE 2: AgRichter Scale (4 panels)")
    logger.info("=" * 60)
    ...
    try:
        ...
        for i, crop in enumerate(crops):
            ...
            # Note: This creates a new figure, we'd need to modify the visualizer
            # to accept an axes parameter. For now, create individual figures.
        ...
        output_path = results_dir / 'figure2_agrichter_scale.png'
